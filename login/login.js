...
$(document).ready(function() { 
    $("#login_btn").on("click", function() { 
      ...
      if(valid){ //validation이 유효하다면, 사용자가 입력한 값들을 가져와 json화
        var cno = $("#cno").val();
        var pw = $("#pw").val();
        var jsonData = JSON.stringify({ "cno": cno, "pw": pw }); //json 형태로 값 넘기기
        $.ajax({ //그 후 ajax로 서버와 통신
          type: "POST", //POST 방식으로 통신
          url: "../login/confirmLogin.php", //confirmLogin.php와 통신
          data: {data : jsonData}, //data는 json화 한 jsonData를 보냄
          success: function(testedName) { //통신 성공
            if(testedName == false){ //아이디 혹은 비밀번호가 틀렸을 경우(로그인 실패)
              alert("아이디 혹은 비밀번호가 틀렸습니다.");
            }
            else{ //아이디와 비밀번호가 일치할 경우(로그인 성공)
